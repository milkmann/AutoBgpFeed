...
**Auto BGP Feed** is a high-performance, automated BGP route feed gateway and manager for selective VPN routing across all major home routers (**MikroTik RouterOS 7/6**, Keenetic, OpenWrt, VyOS/EdgeOS, Cisco IOS, OPNsense/pfSense) and Linux servers.
...
| **Country Feeds** | Hardcoded to single country. | **Universal Country Feeds (`COUNTRY`)** — 1-click addition of any country (`UA`, `PL`, `KZ`, `US`, `DE`...). |
...
| **Multi-User & Security**| Single shared password | **Role-Based Access Control (Admin / Viewer)** + PBKDF2-SHA256 password hashing. |
...
[ Upstream Registries (RIPE NCC / Global DNS / IPverse) ]
              ...
              └─────────────┬─────────────┘
                            ...
                            ▼
     [ MikroTik / Keenetic / OpenWrt / VyOS / Cisco Router ]
```
...
- **`COUNTRY`** — Entire countries (`Ukraine`, `UA`, `Poland`, `PL`, `KZ`, `US`...). Uses official RIR pools with automatic CIDR compression.
- **`DOMAIN`** — Single domain names (`rutracker.org`, `chatgpt.com`). Queries 5 global DNS resolvers and applies Anycast protection.
...
**Auto BGP Feed** — это автоматизированный BGP-шлюз и генератор списков маршрутов для выборочной VPN-маршрутизации на любых домашних роутерах (**MikroTik RouterOS 7/6**, Keenetic, OpenWrt, VyOS/EdgeOS, Cisco IOS, OPNsense/pfSense) и Linux-серверах.
...
> 💡 *Проект вдохновлен концепцией [LeonidOz/BGP-Antifilter](https://github.com/LeonidOz/BGP-Antifilter) и полностью переработан в высокопроизводительную модульную архитектуру с минимальным потреблением ресурсов (~89 MB RAM), динамическим BGP-пулом, страновыми фидами и интеллектуальной CDN-маршрутизацией.*
...
| **Страновые фиды** | Привязаны только к одной стране. | **Универсальные страновые пулы (`COUNTRY`)** — добавление любой страны (`Украина/UA`, `Польша/PL`, `KZ`, `US`...). |
...
| **Сетевая разведка** | Отсутствует | **Network Intelligence**: авто-определение страны, провайдера, ASN, CDN-статуса и ссылки на 5 независимых BGP-агрегаторов. |
| **База маршрутов** | Статический текстовый файл | **Интерактивный Route Explorer** с живым поиском по базе 8.6k+ префиксов. |
| **Пользователи и безопасность** | Один общий пароль в файле | **Управление пользователями (Admin / Viewer)** + PBKDF2-SHA256 хэширование паролей. |
...
```routeros
# 1. Создаем фильтр BGP на MikroTik (замените wireguard1 на имя вашего VPN-интерфейса)
/routing/filter/rule
add chain=antifilter-in disabled=no rule="if (bgp-communities includes 65000:1000) { set gw wireguard1; accept; } else { reject; }"
...
Установите BIRD2 (`opkg install bird2`) и добавьте в `/opt/etc/bird.conf`:
...
protocol bgp bgp_feed {
    ...
    ipv4 {
        import filter {
            if (65000, 1000) ~ bgp_community then {
                gw = 10.0.0.1; # IP вашего VPN-шлюза
                accept;
            }
            reject;
        };
...
Установите BIRD2 (`opkg install bird2`) и добавьте в `/etc/bird.conf`:
...
- **`COUNTRY`** — Страны целиком (`Украина`, `UA`, `Польша`, `PL`, `KZ`, `US`...). Загружает официальный реестр RIR со сжатием CIDR.
- **`DOMAIN`** — Домен сайта (`rutracker.org`, `chatgpt.com`). Опрашивает 5 мировых DNS-резолверов и классифицирует Anycast/CDN.
- **`ASN`** — Автономная система целиком (`AS15169` Google/YouTube, `AS32934` Meta/Instagram, `AS62041` Telegram). Выгружает анонсированные префиксы из RIPEstat.
